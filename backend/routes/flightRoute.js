  ...
  try {
      ...
      if (departureTime) {
          searchCriteria.departureTime = {
              [Op.gte]: new Date(departureTime),  // Cerca voli dalla data specificata in poi
          };
      }

      if (availableSeats) {
          searchCriteria.availableSeats = {
              [Op.gte]: availableSeats,  // Cerca voli con un numero minimo di posti disponibili
          };
      }
      ...
      // Restituisce i voli e i dettagli di paginazione
      res.status(200).json({
          flights,
